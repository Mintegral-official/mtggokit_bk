...
func NewGaugeVec(opts GaugeOpts, logOpts GaugeLogOpts, labelNames []string) *GaugeVec {
    desc := NewDesc(
        BuildFQName(opts.Namespace, opts.Subsystem, opts.Name),
        opts.Help,
        labelNames,
...
func (v *GaugeVec) monitor(second int, logPath string, fqName string) {
    ...
    for {
        <-ticker.C
        //2 is gauge metric
        v.metricVec.metricMap.printLog(gaugeType, gaugeLog)
    }
}
