...
func NewCounterVec(opts CounterOpts, logOpts CounterLogOpts, labelNames []string) *CounterVec {
    desc := NewDesc(
        BuildFQName(opts.Namespace, opts.Subsystem, opts.Name),
        opts.Help,
        labelNames,
...
func (v *CounterVec) monitor(second int, logPath string, fqName string) {
    ...
    for {
        <-ticker.C
        //1 is counter metric.
        v.metricVec.metricMap.printLog(counterType, counterLog)
    }
}
